 ...
 * modified by John Kim to include sendDatafunc reference instead of static data
 */
...
(function($) {
    $.PeriodicalUpdater = function(options, callback){
        ...
        settings = jQuery.extend({
            ...
            sendDatafunc: undefined,           // array of values to be passed to the page - e.g. {name: "John", greeting: "hello"}
...
//             f(settings.url, settings.sendData, function(d){
            f(settings.url, settings.sendDatafunc(), function(d){
                if(prevContent != d)
                {
                    // content has changed
                    prevContent = d;
        ...
        //stopPeriodicalTimer=function(){clearTimeout(PeriodicalTimer);};
        //startPeriodicalTimer=function(){PeriodicalTimer = setTimeout(getdata, settings.minTimeout);};
    };  
})(jQuery);
